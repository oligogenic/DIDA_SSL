...
def main():
    ...
    def LOGO_crossValidation(X, y, X_u, y_u, groups, N=50, threshold=None):
        ...
        for i in range(N):
            ...
            for train_index, test_index in logo.split(X, y, groups):
                ...
                y_u_relab = get_y(
                    concatenate( (X_fit, X_u) ),
                    concatenate( (y_fit, y_u) ),
                    0,
                    100
                )

                X_fit = concatenate( (X_fit, X_u) )
                y_fit = concatenate( (y_fit, y_u_relab) )
