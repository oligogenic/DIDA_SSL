...
def main():
    ...
    def LOGO_crossValidation(X, y, X_sup, y_sup, unlabeled, groups, N=50, threshold=None):
        """ Leave one group out cross validation. Returns best classifier. """

        logo = LeaveOneGroupOut()

        sum_sen, sum_spe, sum_mcc, sum_auc = [], [], [], []
        votes, n_v = zeros( (len(unlabeled), 2) ).astype(float)[:,0], 0
        for i in range(N):
            ...
            for train_index, test_index in logo.split(X, y, groups):
                ...
                u_pred = clf.predict_proba(unlabeled)[:,1]
                votes += u_pred
                n_v += 1
        ...
        votes = 0.5 - votes/n_v
        consensus = max(range(len(votes)), key=lambda i: abs(votes[i]))
        return consensus, int(votes[consensus] > 0)

    while len(X_u):
        print("#"*40)
        print('#'*10, len(X_u), 'unlabeled.')
        print("#"*40)
        i, l_i = LOGO_crossValidation(
            X_l, y_l,
            X_r, y_r,
            X_u,
            gene_pairs,
        ...
        print(i, 'has been chosen to adopt class', l_i)
        X_r = concatenate( (X_r, X_u[i].reshape( (1,12) ) ) )
        y_r.append(l_i)
        X_u = delete(X_u, [i], axis=0)
