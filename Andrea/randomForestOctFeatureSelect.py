...
def crossValidation(sets,vectorDic,labelDic,Forest,featureGlobal):
	...
	for pair in sets:
		...
		pred=Forest.predict_proba(vettoriTest)
		PRED=[]
		for i in pred:
			PRED.append(i[1])
...
for k in vectorDicGDIpair:
	#subVectors[k]=vectorDicGDIpair[k][0:2]+vectorDicGDIpair[k][4:6]+vectorDicGDIpair[k][3:4]+vectorDicGDIpair[k][7:8]#+vectorDicGDIpair[k][8:9]
	subVectors[k]=vectorDicGDIpair[k][0:3]+vectorDicGDIpair[k][4:7]

	print len(subVectors[k])

numAttributi=len(subVectors["dd001"])
...
for i in range(0,folds):
	...
	print i+1,"-fold:"
	res=crossValidation(newset,subVectors,labelDic,Forest,featureGlobal)
